...
function cookieHeader(sessionId: string, request: Request, maxAge = 86400) {
...
export default async (request: Request, context: Context) => {
  ...
  if (action === "status") return json({ connected: Boolean(session?.accessToken), configured, workspaceName: session?.workspaceName });
  ...
  if (action === "callback") {
    ...
    await sessions.setJSON(`session:${sessionId}`, { accessToken: tokenPayload.access_token, workspaceName: tokenPayload.workspace_name });
    return new Response(null, { status: 302, headers: { Location: "/?notion=connected", "Set-Cookie": cookieHeader(sessionId, request) } });
  }

  if (action === "search") {
    if (!session?.accessToken) return json({ error: "Notion is not connected." }, 401);
    const query = new URL(request.url).searchParams.get("q") ?? "";
    const searchResponse = await notionRequest("/search", session.accessToken, { method: "POST", body: JSON.stringify({ query, page_size: 50 }) });
