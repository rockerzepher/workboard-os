...
The browser foundation uses synthetic data and local browser persistence so the operating model can be exercised without touching external sources. Obsidian context can be selected as a browser-local, read-only Markdown preview, and Google Tasks has both a synthetic dry-run preview and an optional browser OAuth path using the read-only Tasks scope; provider write-back, SQLite, refresh-token storage, and scoped Notion references remain staged behind the current interface.
