...
function localTaskFromGoogleRecord(record: GoogleTaskRecord, existing?: Task): Task {
  return {
    id: existing?.id ?? `google-${record.sourceId}`,
    sourceKey: record.sourceKey,
    title: record.title,
    container: existing?.container ?? googleListContainer(record.listName),
  ...
  return (
    <div className={`app-shell ${view === "board" ? "board-mode" : view === "today" ? "today-mode" : ""}`}>
      ...
      <main className="main-canvas">
        ...
        {view === "board" && <BoardOverview tasks={tasks} counts={counts} navigate={navigate} onToggle={toggleTask} onReorder={reorderTask} />}
...
function BoardOverview({ tasks, counts, navigate, onToggle, onReorder }: { tasks: Task[]; counts: Record<string, number>; navigate: (view: View) => void; onToggle: (id: string) => void; onReorder: (id: string, targetId: string | null, targetContainer: Container) => void }) {
  return <div className="content-wrap board-overview-page"><PageHeader eyebrow="WORK BOARD / OVERVIEW" title="Work Board Overview" description="Columns" actions={<button className="button outline small" onClick={() => navigate("settings")}><SettingsIcon size={13} /> Edit</button>} />
    <section className="board-intro"><div><span className="live-indicator" /> LOCAL DEMO DATA · READY TO PLAN</div><div className="board-date">Thursday · 17 August 2026</div></section>
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card">
          ...
          {notionStatus.connected ? <>
            ...
            {notionResults.length > 0 && <div className="notion-reference-list">{notionResults.map((reference) => <div className="notion-reference-item" key={reference.id}><div><strong>{reference.title}</strong><span>{reference.kind}</span></div><button className="button outline small" onClick={() => onNotionAddReference(reference)}>Add scope</button></div>)}</div>}
